<template>
  <div class="table-wrap coltype" style="width: 50%">
    ...
    <br/>
    <div class="form-group">
    <input type="text" class="" v-model="inputSearch" style="width:500px" >
    ...
    <table class="table ui-accordion">
      ...
      <tbody>
        <tr class="head class-checkdata" v-for="(rowData, index) in displayData" :key="index">
          <td v-for="(item, columnIndex) in data.columns" :key="`td-${index}-${columnIndex}`">{{rowData[item.field]}}</td>
        </tr>
      </tbody>
    </table>
  </div>
...
export default {
  ...
  methods: {
    doSearch () {
      const searchStr = this.inputSearch.toLowerCase()
      this.displayData = this.data.rows.filter(item => {
        let isMatchingKey = false
        const columnLength = this.data.columns.length
        for (let i = 0; i < columnLength; i++) {
          const searchResource = item[this.data.columns[i].field].toString().toLowerCase()
          if (searchResource.includes(searchStr)) {
            isMatchingKey = true
            break
          }
        }
  ...
  mounted () {
    ...
    this.displayData = this.data.rows
  },
